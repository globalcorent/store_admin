...
Deno.serve(async req=>{
  ...
  try{
    ...
    const event=await stripe.webhooks.constructEventAsync(await req.text(),signature,secret,undefined,cryptoProvider);
    if(event.type==="checkout.session.completed"){
      const session=event.data.object as Stripe.Checkout.Session;
      const{data:order,error}=await db.from("orders").upsert({user_id:session.metadata?.user_id||null,stripe_session_id:session.id,customer_email:session.customer_details?.email,customer_name:session.customer_details?.name,status:session.payment_status,amount_total_cents:session.amount_total||0,currency:session.currency||"usd",shipping:session.collected_information?.shipping_details||null},{onConflict:"stripe_session_id"}).select("id").single();
