...
function renderCart(){
  const items=detailedCart(),count=items.reduce((sum,item)=>sum+item.quantity,0),merchandise=items.reduce((sum,item)=>sum+item.product.price_cents*item.quantity,0);
  ...
  const savings=merchandise-estimatedAfterDeal,remaining=Math.max(0,7500-estimatedAfterDeal),progress=Math.min(100,estimatedAfterDeal/7500*100),shipping=remaining?795:0,estimatedTotal=estimatedAfterDeal+shipping,candlesNeeded=Math.max(0,3-candleCount);
  ...
  el(".shipping-progress").textContent=remaining?`${money(remaining)} away from free shipping${candleCount>=3?" after savings":""}`:"Free U.S. shipping unlocked";el(".shipping-meter-bar i").style.width=`${progress}%`;
  el(".bundle-progress").textContent=candlesNeeded?`Add ${candlesNeeded} more candle${candlesNeeded===1?"":"s"} to save 15%`:`15% candle savings unlocked`;el(".deal-meter i").style.width=`${Math.min(100,candleCount/3*100)}%`;
  el(".cart-items").innerHTML=items.map(({product,quantity})=>`<article class="cart-item"><img class="cart-thumb" src="${escapeHtml(productImage(product))}" alt="${escapeHtml(product.name)}"><div class="cart-item-copy"><span class="cart-item-category">${escapeHtml(categoryLabel(product.category))}${product.size_label?` • ${escapeHtml(product.size_label)}`:""}</span><h3>${escapeHtml(product.name)}</h3><p>${money(product.price_cents)} each ${isCandle(product)&&candleCount>=3?'<b>15% deal applied</b>':""}</p><div class="qty"><button data-qty="-1" data-id="${escapeHtml(product.id)}" aria-label="Decrease ${escapeHtml(product.name)} quantity">−</button><span aria-live="polite">${quantity}</span><button data-qty="1" data-id="${escapeHtml(product.id)}" aria-label="Increase ${escapeHtml(product.name)} quantity">+</button></div></div><div class="cart-item-end"><strong>${money(Math.round(product.price_cents*(isCandle(product)&&candleCount>=3?.85:1))*quantity)}</strong><button class="remove" data-remove="${escapeHtml(product.id)}" aria-label="Remove ${escapeHtml(product.name)}">Remove</button></div></article>`).join("");
